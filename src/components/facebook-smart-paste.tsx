...
export function FacebookSmartPaste() {
  ...
  const parseSmartPaste = (text: string): ExtractedPost[] => {
    ...
    potentialPosts.forEach(chunk => {
      ...
      // Extract price
      const priceMatch = chunk.match(/(?:₪|ש"ח|שח)\s*(\d{1,2},?\d{3})|(\d{1,2},?\d{3})\s*(?:₪|ש"ח|שח)/);
      ...
      const locationKeywords = [
        'מודיעין', 'ירושלים', 'תל אביב', 'חיפה', 'באר שבע', 'רמת גן',
        'רחוב', 'שכונת', 'ליד', 'קרוב', 'באזור', 'ב'
      ];
  ...
  return (
    <Card className="p-6">
      <div className="space-y-4">
        <div>
          <h3 className="text-lg font-semibold mb-2">הדבקה חכמה מפייסבוק</h3>
          <Alert>
            <AlertDescription>
          ...
          <div className="space-y-4 mt-6">
            ...
            {extractedPosts.map((post, index) => (
              <Card key={index} className="p-4 space-y-2">
                {post.author && (
                  <p className="text-sm text-muted-foreground">מפרסם: {post.author}</p>
                )}
                ...
                <details className="text-sm">
                  ...
                  <p className="mt-2 whitespace-pre-wrap text-xs">{post.text}</p>
                </details>
              </Card>
            ))}
          </div>
